    ...
    <table class="table table-striped data-table table-responsive">
        ...
        @forelse ($sizes as $size)
            <tr>
                ...
                <td class="text-center">{{ $size->size_unit }}</td>
                <td class="text-center">
                    <a class="nav-link edit-button" data-coreui-toggle="modal" data-coreui-target="#editModal" data-url="{{ url('size/'.$size->id) }}" data-name="{{ $size->name }}" data-size-unit="{{ $size->size_unit }}" style="display: inline-block">
                        <svg class="nav-icon" style="width: 24px; height: 24px; border: 1px solid black; padding: 5px; border-radius: 5px;">
                          <use xlink:href="vendors/@coreui/icons/svg/free.svg#cil-pencil"></use>
                        </svg></a>
                    <a class="nav-link delete-button" data-coreui-toggle="modal" data-coreui-target="#deleteModal" data-url="{{ url('delete-size/'.$size->id) }}" data-name="{{ $size->name }}" data-size-unit="{{ $size->size_unit }}" style="display: inline-block">
                        <svg class="nav-icon" style="width: 24px; height: 24px; border: 1px solid black; padding: 5px; border-radius: 5px;">
        ...
        <div class="modal-content">
            ...
            <form method="post" id="edit-modal">
                ...
                <div class="modal-body">
                    ...
                    <label for="staticEmail" class="col-sm-2 col-form-label">Size Unit</label>
                    <input type="text" class="form-control" name="size_unit" id="edit_size_unit">
                </div>
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="{{ url('create-new-size') }}">
                @csrf
                <div class="modal-body">
                    <label for="staticEmail" class="col-sm-2 col-form-label">Name</label>
                    ...
                    <label for="staticEmail" class="col-sm-2 col-form-label">Size Unit</label>
                    <input type="text" class="form-control" name="size_unit">
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-coreui-dismiss="modal">Close</button>
...
<script>
    $('.edit-button').on('click', function () {
        ...
        $('#edit_size_unit').val($(this).attr('data-size-unit'));
    });
