...
<div class="container">
    <div class="float-right">
        <a href="{{ url('create-new-buyer') }}" type="button" class="btn btn-outline-success mb-4">+ New Buyer</a>
    </div>
    <form method="get" class="mb-4">
        <div class="row">
            <div class="col-md-4">
                <input type="text" name="name" placeholder="Enter buyer name to search" class="form-control" value="{{ $name }}">
            </div>
            <div class="col-md-4">
                <input type="text" name="phone" placeholder="Enter buyer phone number to search" class="form-control" value="{{ $phone }}">
            </div>
            <div class="col-md-4">
    ...
    <table class="table table-striped data-table table-responsive">
        ...
        @forelse ($buyers as $buyer)
            <tr>
                ...
                <td class="text-center">{{ $buyer->rating }}</td>
                <td class="text-center">{{ $buyer->country->name }}</td>
                <td class="text-center">
                    @if ($buyer->city)
                        City: {{ $buyer->city }}<br>
                    @endif
                ...
                <td class="text-center">{{ $buyer->is_recommended == 1 ? 'Yes' : 'No' }}</td>
                <td class="text-center">
                    <a class="nav-link" href="{{ url('buyers/'.$buyer->id) }}" style="display: inline-block">
                        <svg class="nav-icon" style="width: 24px; height: 24px; border: 1px solid black; padding: 5px; border-radius: 5px;">
                          <use xlink:href="vendors/@coreui/icons/svg/free.svg#cil-pencil"></use>
                        </svg></a>
                    <a class="nav-link delete-button" style="display: inline-block" data-coreui-toggle="modal" data-coreui-target="#exampleModal" data-url="{{ url("delete-buyer/".$buyer->id) }}">
                        <svg class="nav-icon" style="width: 24px; height: 24px; border: 1px solid black; padding: 5px; border-radius: 5px;">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Are you sure to delete this buyer?
            </div>
