...
<div class="container">
    <div>
        <button type="button" class="btn btn-outline-success mb-4" data-coreui-toggle="modal" data-coreui-target="#addModal" id="add-button">+ New Unit</button>
    </div>
        ...
        @forelse ($units as $unit)
            <tr>
                ...
                <td class="text-center">
                    <a class="nav-link edit-button" data-coreui-toggle="modal" data-coreui-target="#editModal" data-url="{{ url('unit/'.$unit->id) }}" data-name="{{ $unit->name }}" data-unit-unit="{{ $unit->unit }}" style="display: inline-block">
                        <svg class="nav-icon" style="width: 24px; height: 24px; border: 1px solid black; padding: 5px; border-radius: 5px;">
                          <use xlink:href="vendors/@coreui/icons/svg/free.svg#cil-pencil"></use>
                        </svg></a>
                    <a class="nav-link delete-button" data-coreui-toggle="modal" data-coreui-target="#deleteModal" data-url="{{ url('delete-unit/'.$unit->id) }}" data-name="{{ $unit->name }}" data-unit="{{ $unit->unit }}" style="display: inline-block">
                        <svg class="nav-icon" style="width: 24px; height: 24px; border: 1px solid black; padding: 5px; border-radius: 5px;">
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="{{ url('create-new-unit') }}">
                @csrf
                <div class="modal-body">
                    <label for="staticEmail" class="col-sm-2 col-form-label">Name</label>
...
<script>
    $('.edit-button').on('click', function () {
        ...
        $('#edit_unit').val($(this).attr('data-unit-unit'));
    });
